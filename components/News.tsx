...
const News = ({ isAboutVisible }: { isAboutVisible: Boolean }) => {
  return (
    <div className="relative flex flex-col justify-between md:py-[6rem] lg:px-[6rem] px-6">
      ...
      <div className={`mt-6 border-t-[1px] ${!isAboutVisible ? 'border-t-zinc-900' : 'border-t-[#f9cdcd]'} p-0 flex flex-col gap-6`}>
      {
  newsItems.map((item, index) => (
    <div
      key={index}
      className={`pt-4 md:pb-10 pb-2 ${
        index !== newsItems.length - 1 ? "border-b-[1px] border-b-zinc-900" : ""
    ...
    >
      ...
      <div className="md:w-[70%] flex md:flex-row flex-col justify-between md:pt-0 pt-6">
        <h1 className="md:w-[80%] lg:text-5xl md:text-2xl text-xl group-hover:underline font-medium uppercase leading-none">
          {item.title}
          <br />
          <span className="text-sm md:block hidden leading-none">Press 4.16.25</span>
        </h1>
        <div className="md:hidden pt-10 flex justify-between items-center">
          <p className="text-xs uppercase">
            <span className="font-semibold">Press</span> 4.16.25
          </p>
          <p>
        ...
        <p>
          <ArrowRight className="size-8 md:block hidden" />
        </p>
      </div>
    </div>
  ))
